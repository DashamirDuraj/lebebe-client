...
export function ProductCard({ product, onProductClick }: ProductCardProps) {
  ...
  const getSectionStyle = () => {
    ...
    if (cat.includes("dress") || cat.includes("pink"))
      return "from-[#FBD3E9]/20 to-[#FBD3E9]/5";
    if (cat.includes("boy") || cat.includes("blue"))
      return "from-[#A3D9F5]/20 to-[#A3D9F5]/5";
    return "from-[#B8E9D2]/20 to-[#B8E9D2]/5";
  };

  const badges = [];
  if (product.id === "1" || product.id === "2")
    badges.push({ text: "New", color: "bg-[#7CC4E8]" });
  ...
  return (
    ...
    >
      <div
        className={`relative aspect-square bg-gradient-to-br ${getSectionStyle()} rounded-2xl overflow-hidden mb-4 shadow-elevated group-hover:shadow-xl transition-shadow duration-300`}
      >
        <img
          src={product.image}
          alt={product.name}
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between">
          ...
          <ul className="flex gap-1" aria-label="Available colors">
            <li
              className="w-3 h-3 rounded-full bg-[#F7B6C7] border border-neutral-200"
              aria-label="Pink"
            ></li>
            <li
              className="w-3 h-3 rounded-full bg-[#7CC4E8] border border-neutral-200"
              aria-label="Blue"
            ></li>
            <li
              className="w-3 h-3 rounded-full bg-[#B8E9D2] border border-neutral-200"
              aria-label="Mint"
            ></li>
          </ul>
        </div>
