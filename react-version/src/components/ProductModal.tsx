  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl">
        <DialogHeader>
          <DialogTitle style={{fontWeight: 800, fontSize: '1.75rem'}} className="text-purple-600">{product.name}</DialogTitle>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col gap-4">
            <div>
              <Badge variant="secondary" className="rounded-full px-3 py-1" style={{fontWeight: 700}}>{product.category}</Badge>
              <p className="text-3xl text-pink-500 mt-2" style={{fontWeight: 800}}>${product.price.toFixed(2)}</p>
            </div>
            
            <p className="text-gray-600" style={{fontWeight: 600}}>{product.description}</p>

            <div>
              <label className="block mb-2" style={{fontWeight: 700}}>Select Size 👶</label>
              <div className="flex flex-wrap gap-2">
                {product.sizes.map((size) => (
                  <Button
                    ...
                    className={`rounded-full ${selectedSize === size ? 'bg-purple-500 hover:bg-purple-600' : 'border-2 border-purple-300'}`}
